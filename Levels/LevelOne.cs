...
namespace SideScroller
{
    public class LevelOne : GameScreen
    {
        ...
        public override void LoadContent()
        {
            ...
            map.LoadContent(tex1, tex2, tex3, tex4, tex5, tex6, tex7, tex8, tex9, tex10, tex11, tex12, tex13, tex14, tex15);
            ...
            emblem = content.Load<Texture2D>("ShardEmblemLevel1");
            emblemBrack = content.Load<Texture2D>("ShardEmblemTemplate");         
            emblemRec = new Rectangle(120, 120, emblem.Width, emblem.Height);

            music = content.Load<SoundEffect>("Music/gametrack1");
            ...
            scrolling2 = new Scrolling(content.Load<Texture2D>("foreground"), new Rectangle(0, 0, screenW * 3, screenH));
            scrolling3 = new Scrolling(content.Load<Texture2D>("foreground"), new Rectangle(screenW * 3, 0, screenW * 3, screenH));
            scrolling4 = new Scrolling(content.Load<Texture2D>("foreground"), new Rectangle(screenW * 6, 0, screenW * 6, screenH));
                                                       ...
                                                       bool coveredByOtherScreen)
        {
            ...
            if (IsActive)
            {
                ...
                {
                    LoadingScreen.Load(ScreenManager, false, null, new DeadBackgroundScreen(), new LevelOneDeadScreen());
                }
                if (!started)
                {
                    if (musicInst.State == SoundState.Stopped)
                    {
                        musicInst.Volume = 0.75f;
                ...
                if (!paused)
                {
                    ...
                    {
                        LoadingScreen.Load(ScreenManager, false, null, new BackgroundScreen(), new LevelOne());
                    }
        ...
        public void Camera()
        {
            ...
            if (player.cameraPosition.X > screenW / 2)
            {
                ...
                scrolling2.rectangle.X -= (int)6f;
                scrolling3.rectangle.X -= (int)6f;
                scrolling4.rectangle.X -= (int)6f;
            }
            ...
            foreach (Rectangle top in map.shardTop)
            {
                if (top.Intersects(player.footBounds))
                {
                    if (map.hit = true && map.ls == 1)
                    {
                        ...
                        LoadHighData("highscore1.xml");
                        SaveHigh(hdata, "highscore1.xml");
                        Save(data, "leveldata.xml");
                        LoadingScreen.Load(ScreenManager, false, null, new LevelCompleteBackgroundScreen(), new LevelOneCompleteScreen());
                    }
                    else
                        LoadingScreen.Load(ScreenManager, false, null, new DeadBackgroundScreen(), new LevelOneFailedScreen());
                }
            }
