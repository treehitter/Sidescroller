...
{
    public class LevelThree : GameScreen
    {
        ...
        public override void LoadContent()
        {
            ...
            emblem = content.Load<Texture2D>("ShardEmblemLevel3");
            emblemBrack = content.Load<Texture2D>("ShardEmblemlevel2");
                                                       ...
                                                       bool coveredByOtherScreen)
        {
            ...
            if (IsActive)
            {
                ...
                {
                    LoadingScreen.Load(ScreenManager, false, null, new DeadBackgroundScreen(), new LevelThreeDeadScreen());
                }
                if (!started)
                {
                ...
                if (!paused)
                {
                    ...
                    {
                        LoadingScreen.Load(ScreenManager, false, null, new BackgroundScreen(), new LevelThree());
                    }
            ...
            foreach (Rectangle top in map.shardTop)
            {
                if (top.Intersects(player.footBounds))
                {
                    if (map.hit = true && map.ls == 1)
                    {
                        ...
                        LoadHighData("highscore3.xml");
                        SaveHigh(hdata, "highscore3.xml");
                        Save(data, "leveldata.xml");
                        LoadingScreen.Load(ScreenManager, false, null, new LevelCompleteBackgroundScreen(), new LevelThreeCompleteScreen());
                    }
                    else
                        LoadingScreen.Load(ScreenManager, false, null, new DeadBackgroundScreen(), new LevelThreeFailedScreen());
                }
            }
