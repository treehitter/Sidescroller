...
namespace SideScroller
{
    public class LevelTwo : GameScreen
    {
        ...
        public override void LoadContent()
        {
            ...
            emblem = content.Load<Texture2D>("ShardEmblemLevel2");
            emblemBrack = content.Load<Texture2D>("ShardEmblemLevel1");
            emblemRec = new Rectangle(120, 120, emblem.Width, emblem.Height);

            music = content.Load<SoundEffect>("Music/gametrack2");
                                                       ...
                                                       bool coveredByOtherScreen)
        {
            ...
            if (IsActive)
            {
                ...
                {
                    LoadingScreen.Load(ScreenManager, false, null, new DeadBackgroundScreen(), new LevelTwoDeadScreen());
                }
                if (!started)
                {
                ...
                if (!paused)
                {
                    ...
                    {
                        LoadingScreen.Load(ScreenManager, false, null, new BackgroundScreen(), new LevelTwo());
                    }
            ...
            foreach (Rectangle top in map.shardTop)
            {
                if (top.Intersects(player.footBounds))
                {
                    if (map.hit = true && map.ls == 1)
                    {
                        ...
                        LoadHighData("highscore2.xml");
                        SaveHigh(hdata, "highscore2.xml");
                        Save(data, "leveldata.xml");
                        LoadingScreen.Load(ScreenManager, false, null, new LevelCompleteBackgroundScreen(), new LevelTwoCompleteScreen());
                    }
                    else
                        LoadingScreen.Load(ScreenManager, false, null, new DeadBackgroundScreen(), new LevelTwoFailedScreen());
                }
            }
